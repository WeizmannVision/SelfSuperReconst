...
from .misc import cprint1, cprintc, cprintm, os, np, Image, natsorted, glob
import itertools
import torch
...
class KamitaniDataset(Dataset):
    ...
    def __init__(self, roi, sbj_num=3, im_res=112, img_xfm=identity, fmri_xfm=identity, subset_case=TRAIN, test_avg_qntl=.33, select_voxels='', is_rgbd=0):
        ...
        if is_rgbd:
            images_data = np.load(f'data/rgbd_{im_res}_from_224_large_png_uint8.npz')
            if is_rgbd == 1:
                cprintc('(*) RGBD')    
            else:  # Depth only
                cprintc('(*) Depth only')
            ...
            images_data = np.load(f'data/images_{im_res}.npz') 

        dict_loaded = dict(np.load(f'data/sbj_{sbj_num}.npz'))
...
class RGBD_Dataset(Dataset):
    def __init__(self, depth_only=False):
        ...
        self.rgb_images = ImageFolder('data/imagenet/val')
        self.depth_images_paths = natsorted(glob.glob('data/imagenet_depth/val_depth_on_orig_small_png_uint8/*'))
