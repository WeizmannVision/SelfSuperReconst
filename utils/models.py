...
from .misc import cprint1, cprintc, cprintm, np, interpolate, extract_patches, norm_depth_01, tup2list, hw_flatten
import torch
...
class SeparableEncoderVGG19ml(nn.Module):
    def __init__(self, out_dim, random_crop_pad_percent, spatial_out_dim=None, drop_rate=0.25):
        ...
        if FLAGS.is_rgbd:
            ...
            state_dict_loaded = torch.load('data/imagenet_rgbd/{}_best.pth.tar'.format(ckpt_name))['state_dict']
...
class DepthExtractor(nn.Module):
    def __init__(self, img_xfm_norm=identity, model_type=None):
        ...
        if model_type == "large":
            model_path = 'data/model-f6b98070.pt'
            self.model = MiDaS.midas.midas_net.MidasNet(model_path, non_negative=True)
            self.net_input_size = 384
        elif model_type == "small":
            model_path = 'data/model-small-70d6b9c8.pt'
            self.model = MiDaS.midas.midas_net_custom.MidasNet_small(model_path, features=64, backbone="efficientnet_lite3", exportable=True, non_negative=True, blocks={'expand': True})
